...
def dfsCountMultipliedWeights(graph: dict[str, List[Tuple[int, str]]], src: str) -> int:
    currentVertex: str = src
    res: int = 0
    seen = {vertex: False for vertex in graph.keys()}
    seen[src] = True
    ...
    while len(stack) > 0:
        ...
        for weight, neighbor in neighbors:
            ...
            subRes: int = dfsCountMultipliedWeights(graph, neighbor)
            res += (subRes + 1) * weight
    return res
...
print(invertGraph(graph))
print(dfsCountMultipliedWeights(graph, 'shiny gold'))